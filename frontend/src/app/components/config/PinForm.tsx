...
const PinForm = memo(
  ({
    ...
    const handleInputChange = useCallback(
      (value: string, index: number, isConfirm: boolean = false) => {
    ...
    const handleKeyDown = useCallback(
      (e: React.KeyboardEvent, index: number, isConfirm: boolean = false) => {
    ...
    const handlePaste = useCallback(
      (e: React.ClipboardEvent, isConfirm: boolean = false) => {
        e.preventDefault();
    ...
    const validatePin = useCallback((pinValue: string) => {
      ...
      if (
        /012345|123456|234567|345678|456789|567890/.test(pinValue) ||
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        // ✅ OBTENER TOKEN DE DONDE SEA QUE SE ALMACENE
        const token =
        ...
        const response = await fetch(`${apiUrl}/pin/save`, {
          method: "POST",
          headers,
          credentials: "include", // Mantener por si acaso
          body: JSON.stringify({
            pin: fullPin,
          }),
        });
    ...
    // ✅ FUNCIÓN ACTUALIZADA: handleRemovePin
    ...
    const createRefCallback = useCallback(
      (index: number, isConfirm: boolean = false) => {
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
                    ...
                    <li>
                      • Este PIN le ayudará a recuperar su cuenta si olvida la
                      contraseña
                    </li>
                  </ul>
                </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-2 justify-center mb-4">
              {confirmPin.map((digit, index) => (
                <input
                  ...
                  value={digit}
                  onChange={(e) =>
                    handleInputChange(e.target.value, index, true)
                  }
          ...
          <div className="text-center">
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Puede activar o desactivar esta opción en cualquier momento desde
              la configuración
            </p>
          </div>
        </form>
