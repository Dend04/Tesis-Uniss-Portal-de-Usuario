// app/components/forgot-password/TwoFactorVerification.tsx
"use client";
...
export default function TwoFactorVerification({
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/2fa/verify-code`, {
        method: "POST",
      ...
      console.log("🔐 Verificando código 2FA:", response.status);

      if (!response.ok) {
        const errorData = await response.json();
        
        if (response.status === 400 || errorData.error?.includes("código")) {
          if (retryCount < 2) {
            setError(
      ...
      console.log("✅ Verificación 2FA exitosa:", result);

      if (result.success) {
        // Éxito - proceder al siguiente paso
        onVerificationSuccess();
      } else {
      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button
            ...
            className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 px-4 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
        ...
        <div className="text-center pt-4">
          <p className="text-xs text-gray-500">
            ¿Problemas con el código? Verifica que la hora de tu dispositivo esté sincronizada correctamente.
          </p>
        </div>
      </div>
